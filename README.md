...
Deploy path: lint → build → Firebase (functions first) → Vercel picks up the frontend push automatically. CI runs the same lint and build on push and PRs to `main` (`.github/workflows/ci.yml`). For structured UI reviews, use [docs/GEMINI-UI-WALKTHROUGH.md](docs/GEMINI-UI-WALKTHROUGH.md) with Gemini and full-page screenshots.

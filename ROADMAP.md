...
**Out of scope (deferred):** per-jurisdiction tax rates, auto-import from email/Drive, vendors other than Michelin, backfill of historical orders, admin toggle for the DM, multi-DR-per-invoice.

### 🔧 Catalog-buy-to-landed sweep (shipped 2026-05-01)
Follow-up after landed-cost v1. Hooks (`useDashboardSignals`, `useAdvisorSignals`), tires components (`TireRelatedSizes`, `TiresDashboard` margin column), `exportMarginCsv`, and Slack `/stock` `/margins` `/pricecheck` now key margin / cost / profit / `saleForTargetMarginFraction` off `tireLandedBuyNumber` so portal and Slack agree. Sale-price suggestions, listing-AI input, eFleet drift comparisons, and the legacy `financeStats` cost fallback intentionally stayed on catalog (correct semantic).
